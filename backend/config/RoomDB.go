...
func SetupRoomDatabase() {
	...
	// Seed Bookings
	booking1 := entity.Booking{CheckIn: time.Now(), CheckOut: time.Now().Add(48 * time.Hour), CustomerID: &customer1.ID, RoomID: &room101.ID, TotalPrice: 500, EmployeeID: 1}
	booking2 := entity.Booking{CheckIn: time.Now(), CheckOut: time.Now().Add(48 * time.Hour), CustomerID: &customer2.ID, RoomID: &room201.ID, TotalPrice: 1500, EmployeeID: 1}
